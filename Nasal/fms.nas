			...
			if (asel < me.highest_alt) {
				...
				for (var j=i+1;j<fp.getPlanSize()-1;j+=1) {
					if (v_alt.vector[j] < altWP_curr) {
						...
						### TOD distance along route from departure ###
						tod_dist = wp_dist - top_d;
						top_of_descent = tot_dist - tod_dist;
				...
				if (tod_dist != nil
					and tod_dist >= fp.getWP(i).distance_along_route
					and tod_dist < fp.getWP(i+1).distance_along_route
					and flag_tod == 0
					and tod_dist > v) {
